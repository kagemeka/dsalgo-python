...
@nb.njit
def mod_choose(mod: int, fact: np.ndarray, ifact: np.ndarray, n: int, k: int) -> int:
    ok = (0 <= k) & (k <= n)
    return fact[n] * ifact[n - k] % mod * ifact[k] % mod * ok


@nb.njit
def mod_choose_inverse(mod: int, fact: np.ndarray, ifact: np.ndarray, n: int, k: int) -> int:
    ok = (0 <= k) & (k <= n)
    return ifact[n] * fact[n - k] % mod * fact[k] % mod * ok


@nb.njit
def mod_nHk(mod: int, fact: np.ndarray, ifact: np.ndarray, n: int, k: int) -> int:
...
import typing 
@nb.njit 
def pascal_triangle(op: typing.Callable[[int, int], int], n: int) -> np.ndarray:
