...
import numba as nb
import heapq
from kagemeka.dsa.jit.graph_theory.graph import (
    csgraph_is_sorted,
)
...
@nb.njit
def count_paths_bfs(
    n: int, g: np.ndarray, edge_idx: np.ndarray, src: int, mod: int,
) -> tuple[(np.ndarray, ) * 2]:
    inf = 1 << 60
    dist = np.full(n, inf, np.int64)
    dist[src] = 0
...
def dijkstra_dense(g: np.ndarray, src: int) -> tuple[(np.ndarray, ) * 2]:
    ...
    while True:
        u, du = -1, inf
        for i in range(n):
            if not visited[i] and dist[i] < du: u, du = i, dist[i]
        if u == -1: break
        visited[u] = True         
        for v in range(n):
            dv = du + g[u, v]
...
@nb.njit
def dijkstra_sparse(n: int, g: np.ndarray, src: int) -> tuple[(np.ndarray, ) * 2]:
...
def count_paths_dijkstra_sparse(
    n: int, g: np.ndarray, src: int, mod: int,
) -> tuple[(np.ndarray, ) * 2]:
    inf = 1 << 60
    assert inf > g[:, 2].max() * n
    g = g[np.argsort(g[:, 0], kind='mergesort')]
...
@nb.njit
def bellman_ford_sparse(n: int, g: np.ndarray, src: int) -> tuple[(np.ndarray, ) * 2]:
    m = len(g)
    assert g.shape == (m, 3)
    inf = 1 << 60
    assert inf > g[:, 2].max() * n
    ...
    for i in range(m):
        u, v, w = g[i]
        if dist[u] + w < dist[v]: raise Exception('Negative cycle found.')
    return dist, predecessor
...
def floyd_warshall(g: np.ndarray) -> NoReturn:
    ...
    for i in range(n):
        if g[i, i] < 0: raise Exception('Negative cycle found.')
